...
const Loading = ({ className }: { className?: string }) => {
  return (
    <div className={cn("mt-20 flex min-h-0 flex-col flex-wrap items-center justify-center", className)}>
      <div className=" relative h-2  w-[40vw] overflow-clip rounded-sm border-2 bg-primary-foreground">
        <div className="absolute h-full animate-load-bar rounded-sm bg-primary"></div>
        <div
          style={{ animationDelay: `500ms` }}
          className="absolute h-full animate-load-bar rounded-sm bg-primary"
        ></div>
      </div>
