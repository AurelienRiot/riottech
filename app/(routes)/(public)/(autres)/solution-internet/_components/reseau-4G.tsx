  ...
  return (
    <section className="flex flex-wrap justify-between gap-6 p-6">
      {features.map((feature, index) => (
        <div key={index} className="group flex w-96 flex-auto flex-col gap-4" id={feature.title}>
          <Image
            width={100}
            height={100}
            src={feature.image}
          ...
          <h2 className="relative text-center text-xl font-semibold text-primary">
            <HoverWord className="inline-flex items-center p-2">{feature.title}</HoverWord>
          </h2>
          <div className="mx-auto max-w-96 text-center">{feature.description}</div>
        </div>
      ))}
    </section>
  );
...
const features: Feature[] = [
  {
    ...
    description: (
      <p>
        ...
        <br />
        Grâce à cette collaboration, nous pouvons vous proposer des cartes SIM intelligentes, multi-opérateurs, qui
        choisira pour vous le meilleur opérateur à capter selon votre emplacement.
      </p>
    ),
  },
  {
  ...
  },
  {
    ...
    description: (
      <p>
        Une connexion plus fiable et avec une couverture du territoire plus importante que les offres des autres
        opérateurs mobiles.
        <br />
        Un SAV professionnel, sans plateforme téléphonique
        <br />
        Une garantie à vie sur le matériel (
        <Button className={"inline px-0 hover:text-blue-500"} variant={"link"} asChild>
          <Link href={"#La garantie à vie, ça signifie quoi ?"}>Cf. Garantie</Link>
        </Button>
        )
      </p>
    ),
  },
  {
    ...
    description: (
      <p>
        {
          "  Les équipements RIOT TECH sont spécialement conçus pour répondre aux problématiques de milieu auquel peut être confrontée une exploitation agricole par exemple, tels que l'humidité ou les températures extrêmes."
        }
        <br />
  ...
  },
  {
    ...
    altImage: "certificat de garantie",
    title: "La garantie à vie, ça signifie quoi ?",
    description: (
      <p className="relative p-4">
        Routeur garanti à vie, SAV simplifié
        <br />
        En cas de problème sur routeur et après diagnostic par le SAV RIOT TECH, l’envoi d’un équipement de remplacement
        sera effectué et le client sera tenu de renvoyer son équipement défectueux dans le même colis et en utilisant le
        bordereau de
        <HoverWord>retour gratuit.</HoverWord>
        <br />
        La garantie à vie est valable uniquement sur les équipements RIOT TECH avec cartes SIM préinstallées vendus par
        RIOT TECH ou ses partenaires et sous condition de maintien de la souscription à un abonnement RIOT TECH.
      </p>
    ),
  },
];
