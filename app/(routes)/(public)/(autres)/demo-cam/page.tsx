...
const DemoCamPage = () => {
    ...
    <article className="flex flex-wrap items-center justify-center p-4 border-4 border-gray-700 rounded-xl">
      <div className="flex flex-col items-center justify-center w-full p-6 lg:w-1/2">
        ...
        <p className="mb-4 text-lg text-center">
          La caméra RIOT TECH est une caméra de surveillance intelligente conçue
          pour les élevages. Elle offre les avantages suivants :
        </p>
  ...
  {videos.map((video) => (
    <div key={video.id} className="relative grid items-center justify-center grid-cols-1 w-[480px]">
      <h2 className="flex justify-center mt-2 text-2xl font-semibold text-center">{video.nom}</h2>
      <div className="h-[270px]">
        <iframe src={`https://www.youtube.com/embed/${video.id}`} title={video.nom} allowFullScreen className="w-full h-full" />
      </div>
    </div>
  ))}
</section>
