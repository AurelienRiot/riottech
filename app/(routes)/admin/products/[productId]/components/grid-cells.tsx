...
const GridCells = ({
  ...
  const [grid, setGrid] = useState(Array.from({ length: size }, () => Array.from({ length: size }, () => 0)));

  const handleOnMouseOver = (rowIndex: number, colIndex: number) => {
    // Toggle the value (0 or 1) when a cell is clicked
    const newGrid = grid.map((row, rowIdx) =>
      row.map((cellValue, colIdx) => (rowIdx <= rowIndex && colIdx <= colIndex ? 1 : 0)),
    );
    setGrid(newGrid);
  };

  return (
    <div style={{ width: 16.5 * size }} className="flex  flex-wrap flex-row items-center self-center gap-[0.5px]">
