...
const MouseHover = () => {
  ...
  function handleMouseMove(event: React.MouseEvent<HTMLElement> | React.TouchEvent<HTMLElement>) {
    let clientX: number;
    let clientY: number;
    ...
    if (digitsRef.current) {
      ...
      if (distanceToContainer > Math.max(container.width, container.height) * 0.5 + proximityRadius) {
        return;
      }
  ...
  "
      ...
      <section
        className={`grid gap-16 items-center justify-center absolute top-[20%] 
                    left-[10%] `}
      >
