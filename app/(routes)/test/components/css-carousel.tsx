    ...
    <>
      <div className="carousel justify-self-center carousel-center w-[200px] h-[150px] p-4 space-x-4 bg-black rounded-lg hide-scrollbar">
        {carouselItems.map((item, index) => (
          <div
            key={index}
            className="carousel-item w-[120px] h-[120px] relative "
          >
            <Image
              alt={item.alt}
              fill
              sizes="20%"
              src={item.src}
              className="rounded-lg object-contain pointer-events-none box-content border border-gray-700 w-full h-full"
            />
          </div>
        ))}
      </div>
